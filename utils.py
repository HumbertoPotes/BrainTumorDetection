...
def clean_by_distance(pred_mask, threshold=0.5, stdev_multiplier=3.5):
    ...
    total = bin_mask.sum(dim=(1, 2), keepdim=True) + eps # Sum over height and width not batch
    ...
    std_devs = dist_to_center.view(B, -1).masked_select(masked_mask).view(B, -1).std(dim=1, keepdim=True).view(B, 1, 1)
    cleaned = ((bin_mask == 1) & (dist_to_center <= stdev_multiplier * std_devs)).to(torch.uint8)

    return cleaned  

def visualize_comparisons(image_np, pred_mask, pred_cat, true_mask, true_cat, batch_size):
    ...
    for i in range(batch_size):
        ...
        axs[i,1].imshow(overlay, alpha=0.7, label="Predicted Mask")  # Add transparency to the overlay
        # original mask
        overlay = empty((640, 640, 3))
    ...
    fig.suptitle(f"""Visualization of the Network's Predictions
